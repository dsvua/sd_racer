...
namespace ORUtils
{
	...
	struct Matrix4_
	{
		...
		{
			struct
			{						  // Warning: see the header in this file for the special matrix order
	...
	public:
		...
		_CPU_AND_GPU_CODE_ inline Matrix3_<T> getRotation() const { return Matrix3_<T>(this->m00, this->m01, this->m02,
																					   this->m10, this->m11, this->m12,
																					   this->m20, this->m21, this->m22); }
		_CPU_AND_GPU_CODE_ inline Vector3<T> getTranslation() const { return Vector3<T>(this->m03, this->m13, this->m23); }
		...
		_CPU_AND_GPU_CODE_ inline void setValues(T r)
		{
			for (int i = 0; i < 16; i++)
				this->m[i] = r;
		}
		...
		_CPU_AND_GPU_CODE_ inline void setTranslate(const Vector3_<T> &t)
		{
			for (int y = 0; y < 3; y++)
				at(3, y) = t.v[y];
		}
		_CPU_AND_GPU_CODE_ inline void setTranslation(const Vector3_<T> &t)
		{
			for (int y = 0; y < 3; y++)
				at(3, y) = t.v[y];
		}
		_CPU_AND_GPU_CODE_ inline void setRotation(const Matrix3_<T> &t)
		{
			this->m00 = t.m00;
			this->m01 = t.m01;
			this->m02 = t.m02;
		...
		_CPU_AND_GPU_CODE_ inline void setRow(int r, const Vector4_<T> &t)
		{
			for (int x = 0; x < 4; x++)
				at(x, r) = t.v[x];
		}
		...
		_CPU_AND_GPU_CODE_ inline Matrix4 t() const
		{ // transpose
			Matrix4 mtrans;
			for (int x = 0; x < 4; x++)
				for (int y = 0; y < 4; y++)
	...
	public:
		...
		_CPU_AND_GPU_CODE_ inline void setValues(const T r)
		{
			for (int i = 0; i < 9; i++)
				this->m[i] = r;
		}
		...
		_CPU_AND_GPU_CODE_ inline void setRow(int r, const Vector3_<T> &t)
		{
			for (int x = 0; x < 3; x++)
				at(x, r) = t[x];
		}
		...
		_CPU_AND_GPU_CODE_ inline Matrix3 t()
		{ // transpose
			Matrix3 mtrans;
			for (int x = 0; x < 3; x++)
				for (int y = 0; y < 3; y++)
	...
	public:
		...
		// set values
		_CPU_AND_GPU_CODE_ inline void setValues(const T *mp)
		{
			for (int i = 0; i < s * s; i++)
				this->m[i] = mp[i];
		}
		...
		_CPU_AND_GPU_CODE_ inline void setZeros()
		{
			for (int i = 0; i < s * s; i++)
				this->m[i] = 0;
		}
